...
#            **config['optimizer'])
        ...
        for i in range(self.num_iters):
            ...
            if self.iter % self.print_every == 0:
                info = 'iter: {:06d} - train loss: {:.3f} - lr: {:.2e} - load time: {:.2f} - gpu time: {:.2f}'.format(
                    self.iter, total_loss / self.print_every, self.optimizer.param_groups[0]['lr'], total_loader_time, total_gpu_time)
                ...
                epoch_weight = self.export_weights[:-4] + \
                    '_' + str(self.iter/200) + '.pth'
                self.save_weights(epoch_weight)
    ...
    def test(self):
        if self.valid_annotation and self.iter % self.valid_every == 0:
            ...
            info = 'valid loss: {:.3f} - acc full seq: {:.4f} - acc per char: {:.4f}'.format(
                self.iter, val_loss, acc_full_seq, acc_per_char)
            print(info)
            return {
                "val_loss": val_loss,
